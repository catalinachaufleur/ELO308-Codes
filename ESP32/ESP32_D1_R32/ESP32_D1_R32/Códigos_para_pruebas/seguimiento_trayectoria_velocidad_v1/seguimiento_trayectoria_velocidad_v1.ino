...
//turn the PID on
  ...
  PID_theta.SetOutputLimits(-300,300);
//
//turn the PID on
  PID_vel.SetMode(AUTOMATIC);
  PID_vel.SetSampleTime(SampleTime_vel);
  PID_vel.SetOutputLimits(-300,300);
//
  pinMode(26, INPUT);
...
Input_vel=PI*radio_rueda*(encoder_der.read()+encoder_izq.read())*1000/(SampleTime_vel*50*12); // ENCODERS:12CPR---CAJA_REDUCTORA:50:1
PID_theta.Compute();
if(PID_vel.Compute()){
  encoder_der.write(0);
  encoder_izq.write(0);
