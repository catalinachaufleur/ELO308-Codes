...
void loop() {
    ...
    //Serial.println("Esperando Sync!");
  //  delay(50);
    myPID_v.SetMode(MANUAL);
    myPID_v.SetMode(MANUAL);
    ...
    {   
        ...
        */
        ...
        t_old = millis();
               ...
               estado = String(time_now)+","+String(medi)+","+String(x_ref)+","+String(error_distancia)+","+String(Kp)+","+String(Ki)+","+String(Kd)+","+String(MotorSpeed);
                estado.toCharArray(msg, estado.length() + 1);                                                                           
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if (strcmp(topic , "trenes/motor") == 0) {
        MotorSpeed = mensaje.toInt();
